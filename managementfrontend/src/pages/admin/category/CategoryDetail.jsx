...
const CategoryDetail = () => {
  ...
  const columns = [
    { header: "ID", accessor: "id" },
    {
      ...
      render: (row) => {
        const imageUrl = row.mainImageUrl 
          ? (row.mainImageUrl.startsWith("http") ? row.mainImageUrl : `http://localhost:8080/images/${row.mainImageUrl}`) 
          : null;
        ...
        return (
          <div className="product-thumbnail">
            {imageUrl ? (
              ...
              <div 
                style={{ width: "50px", height: "50px", backgroundColor: "var(--bg-secondary)", borderRadius: "6px", display: "flex", alignItems: "center", justifyContent: "center", fontSize: "10px", color: "var(--text-secondary)", textAlign: "center" }}
              >
                No Image
              </div>
            )}
    ...
    },
    {
      ...
      accessor: "status",
      render: (row) => (
        <span className={`status-badge ${row.status?.toLowerCase() || 'inactive'}`}>
  ...
  if (!category) {
    return <div className="loading-text">Không tìm thấy thông tin danh mục.</div>;
  }
    ...
    <div className="category-detail-container">
      ...
      <div className="cg-info-card">
        <div className="cg-title">
          <h2 className="cg-info-title">Chi tiết danh mục: {category.name}</h2>
        ...
        <div className="cg-info-grid">
          <div className="cg-info-item">
            ...
            <span className="cg-info-value">{category.description || "Chưa cập nhật"}</span>
          </div>
          <div className="cg-info-item">
            <span className="cg-info-label">Số lượng sản phẩm:</span>
            <span className="cg-info-value">{category.productCount || 0}</span>
          </div>
          <div className="cg-info-item">
            <span className="cg-info-label">Trạng thái:</span>
            <span className={`status-badge ${category.categoryStatus?.toLowerCase()}`}>
              {category.categoryStatus === "active" ? "Hoạt động" : "Tạm ngưng"}
            </span>
          </div>
          <div className="cg-info-item">
